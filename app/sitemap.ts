...
export default async function sitemap():Promise<MetadataRoute.Sitemap>{
  ...
  if(!hasSupabasePublicEnv())return entries;const supabase=await createClient();const [doctors,articles,services]=await Promise.all([supabase.from("doctors").select("slug,updated_at").eq("is_active",true).is("deleted_at",null),supabase.from("articles").select("slug,updated_at").eq("is_active",true).is("deleted_at",null).lte("published_at",now.toISOString()),supabase.from("services").select("slug,updated_at").eq("is_public",true).is("deleted_at",null)]);
