...
export const metadata: Metadata = { title: "الأسئلة الشائعة | FAQ" };

export default async function FaqPage() {
  const locale = await getLocale(); const en = locale === "en"; let items: FaqRow[] = []; let unavailable = false;
  if (hasSupabasePublicEnv()) { const supabase = await createClient(); const { data, error } = await supabase.from("faq_items").select("*").eq("is_active", true).is("deleted_at", null).order("display_order"); items = (data ?? []) as FaqRow[]; unavailable = Boolean(error); } else unavailable = true;
